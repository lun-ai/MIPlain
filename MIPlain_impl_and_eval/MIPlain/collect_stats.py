...
def get_inference():
    ...
    open('./output/rule_wise_inference_MIPlain.txt', 'w').close()
    open('./output/rule_wise_inference_MIGO.txt', 'w').close()
...
def plot_rule_inference_graph():
    l1 = []
    l2 = []
    with open('./output/rule_wise_inference_MIPlain.txt', 'r') as f:
        for line in f:
            l1.append(list(map(float, line.strip('\n').strip().strip('[').strip(']').split(','))))

    with open('./output/rule_wise_inference_MIGO.txt', 'r') as f:
        for line in f:
    ...
    plt.plot(np.arange(0, 101, 1), np.array(p2[0]), label='MIPlain win 1')
    plt.plot(np.arange(0, 101, 1), np.array(p2[1]), label='MIPlain win 2')
    plt.plot(np.arange(0, 101, 1), np.array(p2[2]), label='MIPlain win 3')
...
def plot_inference_graph():
    ...
    plt.plot(np.arange(0, 101, 1), np.array(a) / 1000, 'r', label='MIPlain')
...
def get_regrets():
    open('./output/MIPlain.txt', 'w').close()
    with open(os.devnull, "w") as f:
        for i in range(20):
...
def plot_regret_graph():
    ...
    DQN = []
    MIGO = []
    with open('./output/MIPlain.txt', 'r') as f:
        for line in f:
