...
%% backtrack and draw
%update_exs(backtrack,[B1|Seq],d,Sw,Sd) :-

%% backtrack and lose.
%update_exs(backtrack,[B1|Seq],M,Sw,Sd) :-
%    mark(opponent,M),
...
%% no identical backtrack records
%%update_backtrack(InitialB,positive,G,[B1,B2|Game]):-
%%    depth([B1,B2|Game],Depth),
%%    mark(learner,M),
%%    mark(opponent,M1),
%%    example(K),
...
%%    append(G_head,[B2|Game],G),
%%    (\+ backtrack(_,InitialB,_,G_head,Depth,_)
%%        ->
%%            asserta(backtrack(K,InitialB,M,G_head,Depth,0))
%%        ;
%%            true),
%%    update_ex(Ex2,positive,P),
%%    update_backtrack(InitialB,positive,G,Game).
...
goal(N) :-
    open('./output/MIGO_with_features.txt',append,Out),
    regret_record(Out,N),
    write(Out,',\n'),close(Out).
